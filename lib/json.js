...
var validators = [name]; //other validators
...
function validate(json) {
    ...
    function cb(err) {
        if (err) {
            throw createError(err, 'EINVALID');
        }
        if (--n > 0) return;
        done(json);
    }
...
function name(json, cb) {
    if (!json.name) {
        return cb('No name property set', json);
    }

    if (json.name.length > 50) {
        return cb('The name is too long. 50 character should be more than enough', json);
    }

    if (/[A-Z]/.test(json.name)) {
        throw createError('The name contains upper case letters', json);
    }

    if (!/^[a-z]/.test(json.name)) {
        throw createError('The name has to start with a lower case character from a to z', json);
    }

    if (!/[a-z]$/.test(json.name)) {
        throw createError('The name has to end with a lower case character from a to z', json);
    }
